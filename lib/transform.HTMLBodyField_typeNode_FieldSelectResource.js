...
module.exports = function HTMLBodyField_typeNode_FieldSelectResource(db, transform, input, render, callback){
	...
	db.find({type:range, subject:{$exists:true}}, queryRange).forEach(function(doc){
		...
		if(inputResource.selectMulti){
			var selected = (value===subject)?' checked="checked"':'';
			options.push('<label><input type="checkbox" name="'+escapeHTMLAttr(name)+'.'+(i)+'.value" value="'+escapeHTMLAttr(subject)+'"'+selected+' /><input type="hidden" name="'+escapeHTMLAttr(name)+'.'+(i++)+'.format" value="URI"/>'+escapeHTML(label)+'&lt;'+escapeHTML(doc.subject)+'&gt;</label>');
		}else{
