...
module.exports = function(db, transform, input, render, callback){
	...
	var collection = list.query && list.query.collection;
	if(typeof collection=='string') collection=input['db-mongodb'].collection(list.query.collection);
	if(!collection) collection=input['db-mongodb-nodes'];
	if(!collection) return void callback(new Error('No collection specified'));
	...
	if(list.query&&list.query.sort){
		...
		// Put array of orders into [key,order] form
		query.sort(keys.map(function(k){ return [ k[0]||k.key, k[1]||k.dir||1 ]; }));
	}
