/*
Transform:DocumentHTML_Body_typerdfsResource_Post
	a view:ModuleTransform ;
	view:module "magnode/transform.DocumentHTML_Body_typerdfsResource_Post" ;
	view:range type:HTTPResponse ;
	view:domain type:Document_Post .
*/
...
module.exports = function(db, transform, input, render, callback){
	...
	authz(input.auth, "PUT", input, function(result){if(result){
		...
		//*/
		response.end("transform.DocumentHTML_Body_typerdfsResource_Post: Update <"+input.resource+">\n");
		callback({"http://magnode.org/HTTPResponse":303});
	}else{
		response.statusCode = 403;
		response.setHeader("Content-Type", "text/plain");
		response.end("transform.DocumentHTML_Body_typerdfsResource_Post: Update <"+input.resource+">: Denied\n");
		callback({"http://magnode.org/HTTPResponse":403});
	}});
}
module.exports.URI = "http://magnode.org/transform/DocumentHTML_Body_typerdfsResource_Post";
