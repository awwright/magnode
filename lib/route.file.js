...
var InformationResource = require('./ir');

// TODO: Add an "index" option to optionally handle requests on a directory
// TODO: Add an "autoindex" option to enable/disable production of the DirectoryList resource
// TODO: Add an option for looking up the literal requested file first, then check sourceExtensions
	...
	function routeResource(resource, callback){
		...
		fs.stat(reqFile, function(err, stat){
			if(stat && stat.isDirectory()){
				if(reqFile[reqFile.length-1]!=='/'){
					result['http://magnode.org/LocationRedirect'] = path.basename(reqFile)+'/';
		...
		function statNextFile(i){
			...
			fs.readFile(srcFile, function(err, contents){
				...
				result[srcType] = new InformationResource(contents.toString(), 'text/plain', srcURI);
				result['http://magnode.org/HTMLBodyBlock_ResourceMenu'] = [];
				result.variant = variant;
				callback(null, result);
			});
