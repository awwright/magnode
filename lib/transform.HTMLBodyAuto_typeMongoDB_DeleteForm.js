...
module.exports = function(db, transform, resources, render, callback){
	...
	var schema = resources['http://magnode.org/MongoDBDocumentRequest'].schema;
	...
	var result =
		'<form action="'+escapeHTMLAttr(relativeURI(resources.rdf, resources.request.uri, action.toURI()))+'" method="post">'
		...
		+ '<input type="hidden" name=":if-match" value="'+escapeHTMLAttr(node[schema.etagField]||"")+'"/>'
