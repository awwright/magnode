...
module.exports = function(db, transform, resources, render, callback){
	...
	function haveRenderedForm(err, rendered){
		...
		if(resources.createNew){
			// FIXME _subject defines the old resource name, not a new one!!!
			return void callback(new Error('TODO need to calculate resource URI'));
		}
		var resource = (resources.createNew?fieldData._subject:request.url)
		inputs.request = {url:request.url, headers:headers};
		inputs.response = {setHeader:function(){}, write:function(){}, end:function(v){console.log('PUT:	',v)}};
