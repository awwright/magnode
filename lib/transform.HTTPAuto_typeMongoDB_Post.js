...
module.exports = function(db, transform, input, render, callback){
	...
	var requestMethod = (fieldData._method || input.request.method).toUpperCase();
	...
	resources['http://magnode.org/fieldpost/Object'] = {name:''};
	...
	if(contentTypeProfile!==resourcesTypes[0]){
		return void haveResponse(new Error('Entity profile does not match native profile'));
	}

	function parseFormAuth(){
		var authTest = Object.create(input);
		authTest._auth = fieldData._auth;
		authz.test(authTest, ['parse'], input, function(authorized){if(authorized===true){
			render.render(targetType, resources, transformTypes, haveRenderedForm);
		}else{
			response.statusCode = 403;
			response.end("transform.HTMLBodyAuto_typeMongoDB_Post: Parse entity body to <"+input.resource+">: Denied\n");
	...
	function haveSchema(err, typeDoc){
		...
		function writeInstance(instance, schema, options, ctx){
			if(schema.tokenPattern){
				...
				if(schema.format=='uri'){
					value = url.resolve(input.rdf.resolve(':'), value).toString();
				}
				return value;
			}
			if(schema.type==='shadow'){
				// FIXME this is a hack... how do we know we're really on the "password" field? We don't.
				var oldShadow = input.node.password;
				instance._id = new ObjectId;
				operations.push({collection:input['db-mongodb-shadow'], insert:instance});
				// operations.push({collection:'shadow', remove:{_id: oldShadow}});
				return instance._id;
			}
			if(!schema.put) return instance;
			var collection = (schema&&schema.mongoCollection) || input['db-mongodb-nodes'];
		...
		try {
			...
			if(requestMethod=='PUT'){
				...
				parsedData.$subject = input.resource;
			}
