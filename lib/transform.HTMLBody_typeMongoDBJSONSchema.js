...
module.exports = function HTMLBody_typeMongoDBJSONSchema(db, transform, input, render, callback){
	...
	function haveRenderedForm(err, resources){
		...
		newRef.params = Object.create(newRef.params);
		newRef.params.new = true;
		newRef.requiredTypes = ['http://magnode.org/HTMLBody_PutForm'];
		var menu = '<h1>Schema: '+escapeHTML(input.node&&input.node.label||'')+'</h1>';
		menu += '<div><a href="'+escapeHTMLAttr(relativeuri(input.rdf, newRef.toURI()))+'">Create new instance</a></div>';
		menu += '<dl>';
