...
module.exports.scanDirectorySync = function(dir, render){
	...
	for(var i=0; i<readdirFiles.length; i++){
		...
		if(module instanceof widgetDefinition){
			...
			importTransform(module.makeViewTransform());
...
// There's several ranges of URIs used for widget creation
...
widgetDefinition.prototype.makeViewTransform = function(){
	if(!this.html) return;
	var self = this;
	function viewTransform(db, transform, input, render, callback){
		...
		try{
			var value = self.html(inputElement.name, inputElement.value, inputElement, cb);
		}catch(e){
			return void callback(e);
		}
		if(self.html.length <= 3) cb(null, value);
		function cb(err, value){
			if(err) return void callback(e);
			var ret = {};
...
widgetDefinition.prototype.makeFormTransform = function(){
	...
	function viewTransform(db, transform, input, render, callback){
		...
		var inputElement = input[self.fieldData];
		try{
			var value = self.form(inputElement.name, inputElement.value, inputElement, cb);
		}catch(e){
			return void callback(e);
		}
		if(self.html.length <= 3) cb(null, value);
		function cb(err, value){
			if(err) return void callback(e);
			value += '<input type="hidden" name="'+escapeHTMLAttr(inputElement.name)+'.format" value="'+self.fieldName+'"/>';
			var ret = {};
			outputTypes.forEach(function(v){ret[v] = value;});
			callback(null, ret);
		}
...
widgetDefinition.prototype.makePostTransform = function(){
	...
	function postTransform(db, transform, input, render, callback){
		...
		try{
			var value = self.parseForm(inputElement.name, inputData, cb);
		}catch(e){
			return void callback(e);
		}
		if(self.html.length <= 2) cb(null, value);
		function cb(err, value){
			if(err) return void callback(e);
			var ret = {};
			outputTypes.forEach(function(v){ret[v] = value;});
