...
module.exports = function registerHandler(route, resources, render, root, base){
	...
	var basePrefix = url.resolve(prefix, base||'/');

	// This is the function that the resource router will call when determining who can process the request
	function routeResource(request, callback){
		// Code if there's a request
		var urlArgs = url.parse(request.url, true);
		var requesturl = url.resolve(prefix, urlArgs.pathname);
		if(requesturl.substr(0, basePrefix.length)!==basePrefix) return void callback(null);
		...
		switch(request.method){
			...
			case "HEAD":
				break;
			default:
				// We probably should send a 405 because the resource exists... Nah
				return void callback(null);
		}

		var filepath = root + requesturl.substring(basePrefix.length);
		if(filepath.indexOf('/.')>=0) return void callback(null);
