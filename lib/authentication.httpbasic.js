...
module.exports.prototype.test = function bearerTest(user, actions, resources, callback){
	...
	if(!request) return void callback(false, 'httpbasic:!request');
	...
	this.authenticateRequest(request, function(err, userInfo){
		if(userInfo && userInfo.id){
			...
			var uid = Object.create(user);
			uid.authentication = userInfo;
			authz.test(uid, actions, resources, callback);
		}else{
			var realm = {http:'Basic realm="'+JSON.stringify(realm)+'"'};
			return void callback(false, 'httpbasic:!userInfo', [realm]);
		}
	});
}
...
module.exports.prototype.authenticateRequest = function(request, callback){
	// Cache the results of authentication - FIXME enable this
