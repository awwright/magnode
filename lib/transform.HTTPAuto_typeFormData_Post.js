...
module.exports = function(db, transform, resources, render, callback){
	...
	function parseFormAuth(err, rendered){
		...
		resource = fieldData._subject || resources.resource;
		if(!resource){
	...
	function haveRenderedForm(err, rendered){
		...
		else if(fieldData._etag) headers['if-match'] = '"'+fieldData._etag+'"';
		...
		var write = function write(v){ body += v; };
		var addHeader = function addHeader(n, v){ body += n+': '+v+'\n'; };
		inputs.response = {setHeader:addHeader, addHeader:addHeader, write:write, end:write};
