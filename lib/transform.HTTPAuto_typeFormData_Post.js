...
module.exports = function(db, transform, resources, render, callback){
	...
	if(contentTypeProfile!==resourceTypes[0]){
		// FIXME we might purposefully send a different resource type than the native type, because we'll convert it later on
		return void processResponse(new Error('Entity profile '+contentTypeProfile+' does not match native profile '+resourceTypes[0]));
	}
