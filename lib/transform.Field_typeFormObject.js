...
module.exports = function(db, transform, input, render, callback){
	...
	function parseFields(fieldList){
		...
		if(format=='ObjectProperty'){
			propertyName = fieldData[baseName+fieldName+'.name'];
			if(!propertyName || propertyName.length===0) return void parseFields(fieldList);
			value = fieldData[baseName+fieldName+'.value.value'];
			format = fieldData[baseName+fieldName+'.value.format']||'undefined';
