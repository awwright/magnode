...
module.exports = function(db, transform, input, render, callback){
   ...
   if(cache[templateFile]) {
		try{
			cache[templateFile](scope, helpers, applyResult);
		}catch(e){
			callback({error:e});
		}
		return;
	...
	fs.readFile(templateFile, 'utf8', function(err, str){
		...
		//try{
			...
			for(var i=0; i<queries.length; i++){
				...
				var result = queryResult[i] = input.db.evaluateQuery(queryGraph, "_:query", input);
			...
			for(var i=0; i<queries[0].vars.length; i++){
				...
				console.error("Defining "+queries[0].vars[i]+" = "+input[queries[0].vars[i]]);
			}
