...
jadeRDFaCompiler.prototype.visitTag = function(tag){
	...
	if(name=='value-of'){
		...
		// Block support
		if (tag.block) {
			if (!tag.buffer) this.buf.push('console.error("\\nLOCAL:");console.error(locals);console.error(jadeCalculateParentVarValues(arguments.callee.queries['+foreachIndex+'].parent, locals));console.error(queryResult['+foreachIndex+']);var rows=queryResult['+foreachIndex+'][jadeCalculateParentVarValues(arguments.callee.queries['+foreachIndex+'].parent, locals)]||[]; for(var i=0;i<rows.length;i++) (function(locals){with(locals){\n');
		...
		this.buffer('<' + name);
		blockProperties=this.visitAttributes(tag.attrs);
		this.terse
			? this.buffer('>')
			: this.buffer('/>');
	} else {
		...
		if (tag.attrs.length) {
			this.buffer('<' + name);
			if (tag.attrs.length) blockProperties=this.visitAttributes(tag.attrs);
			this.buffer('>');
		} else {
			this.buffer('<' + name + '>');
		}
		...
		this.visit(tag.block);
		if(blockProperties){
		//	console.log(util.inspect(blockProperties));
		}
		this.buffer('</' + name + '>');
	}
}
jadeRDFaCompiler.prototype.visitAttributes = function(attrs){
	...
	attrs.forEach(function(attr){
		if (attr.name == 'class') {
			...
			if(attributes.hasOwnProperty(attr.name)) attributes[attr.name]=attr.val;
...
function renderWithQuery(fn, scope, helpers, templateOutputType, callback){
	...
	for(var i=0; i<queries.length; i++){
		...
		//console.error('\x1b[1mQuery Graph\x1b[0m:\n%s\n', util.inspect(queryGraph,false,null).replace(/^/gm, '\t'));
		var result = queryResult[i] = scope.input.db.evaluateQuery(queryGraph, "_:query", {});
...
module.exports = function(db, transform, input, render, callback){
	...
	var ret;
	var scope = {input:input};
	var helpers = {__:{lineno:1}, attrs:jadeAttrs, escape:jadeEscape, jadeCalculateParentVarValues:jadeCalculateParentVarValues};

   if(cache[templateFile]) {
		try{
			renderWithQuery(cache[templateFile], scope, helpers, templateOutputType, callback);
		}catch(e){
			callback({error:e});
		}
		return;
	...
	fs.readFile(templateFile, 'utf8', function(err, str){
		...
		//try{
			...
			//if(templateFile=="template/DocumentHTML_BodyPage_typePage.jade")
				console.error('\n\x1b[1mParsed Nodes\x1b[0m:\n%s\n\x1b[1mDatabase Query\x1b[0m:\n%s\n\x1b[1mCompiled Function\x1b[0m:\n%s\n', util.inspect(parserOut,false,null).replace(/^/gm, '\t'), util.inspect(queries,false,null).replace(/^/gm, '\t'), funcBody.replace(/^/gm, '\t'));
			var fn = cache[templateFile] = new Function('locals', 'helpers', 'callback', funcBody);
			fn.queries = queries;
			renderWithQuery(fn, scope, helpers, templateOutputType, callback);
		//}catch (e) {
		//  return callback({error:e});
		//}
	});

	/*jade.renderFile(templateFile, {cache:module.exports.cache, locals:{input:input}, compiler:jadeRDFaCompiler}, function(err, result){
		if(err){
