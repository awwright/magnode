...
module.exports = function(db, tid, input, render, callback){
	...
	var renderTypes = db.match(tid,"http://magnode.org/view/templateRange").map(function(v){return v.object;});
	...
	jade.renderFile(templateFile, locals, function(error, result){
		...
		var output = {};
		renderTypes.forEach(function(v){
			switch(v.toString()){
		...
		if(titleOutputType){
			output['http://magnode.org/DocumentTitle'] = input.node && input.node[titleOutputType] || tid;
		}
