...
module.exports = function(config){
	...
	this.secret = config.secret || crypto.randomBytes(40);
	if(typeof this.secret=='string') this.secret=new Buffer(this.secret, 'binary');
	if(typeof this.secret.length!='number' || this.secret.length < 16) throw new Error("There's no way this secret key is secure");
...
module.exports.prototype.authenticateRequest = function(request, callback){
	...
	hash.update(body);
	if(hash.digest("base64").replace(/=/g,'')!==tokenParts[0]) return false;
