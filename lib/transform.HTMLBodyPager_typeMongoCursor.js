...
module.exports = function(db, transform, input, render, callback){
	...
	function finishedPosts(err, resultCount){
		...
		function getOffsetURI(variant, offset){
			...
			return alternate.toURI();
		}

		result += '<div id="paginator"><div class="pagination">';
		if(currentPage<=0) result += '<span class="disabled">&lt;&lt;</span>&nbsp;&nbsp;';
		else result += '<a href="'+escapeHTMLAttr(getOffsetURI(input.variant, (currentPage-1)*resultLimit))+'">&lt;&lt;</a>&nbsp;&nbsp;';
		var displayPages = [0];
		...
		displayPages.sort(function(a,b){return a-b;}).forEach(function(i,j){
			...
			else result += '<a href="'+escapeHTMLAttr(getOffsetURI(input.variant, i*resultLimit))+'">'+(i+1)+'</a>';
