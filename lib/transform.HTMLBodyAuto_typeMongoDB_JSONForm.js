/*
e.g. Transform:HTMLBodyAuto_typeType_JSONForm
	a view:ModuleTransform, view:PutFormTransform ;
	view:module "magnode/transform.HTMLBodyAuto_typeMongoDB_JSONForm" ;
...
module.exports = function(db, transform, resources, render, callback){
	...
	var json = resources['http://magnode.org/DocumentJSON'];
	var fields = [];
	var tail = [];
	...
	function haveRenderedForm(err, res){
		...
		var putFn = Object.create(resources.variant.createTarget || resources.variant); // If this is a new resource, createTarget will specify where to POST it
		putFn.requiredTypes = ['http://magnode.org/HTTPResponse_PutFn'];
		var result =
			'<form action="'+escapeHTMLAttr(relativeURI(resources.rdf, resources.request.uri, putFn.toURI()))+'" method="post">'
			+ '<textarea class="field-json">' + escapeHTML(json) + '</textarea>'
