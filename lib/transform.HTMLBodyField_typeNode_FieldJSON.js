...
module.exports = function HTMLBodyField_typeNode_FieldJSON(db, transform, input, render, callback){
	...
	var out = '<textarea name="'+escapeHTMLAttr(name)+'.value" class="field-json">'+escapeHTMLAttr(JSON.stringify(value,null,"\t"))+'</textarea>';
	out += '<input type="hidden" name="'+escapeHTMLAttr(name)+'.format" value="FormFieldElementJSON"/>';
	var ret = {};
