...
var route = module.exports.create = function MongoDBRouter(query, types){
	if(!types) types = function types(node){ return node.type; };
	return function registerHandler(route, resources, renders){
		function routeResource(resource, callback){
			...
			query(input, function(err, node){
				...
				var result = {};
				var nodeTypes = (typeof types=='function')?types(node):types;
				if(nodeTypes instanceof Array) nodeTypes.forEach(function(v){ result[v] = node; });
				result.node = node;
