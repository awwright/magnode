...
ShadowStore.prototype.authenticateCredential = function(credential, callback){
	...
	db.findOne(query, function(err, doc){
		if(err){
			...
			return void lengthyReturn(false);
		}
		if(!doc || !doc.password){
			return void lengthyReturn(false);
		}
		shadowDb.findOne({_id:doc.password}, function(err, shadow){
			...
			comparePassword(record, credential, function(s){
				if(s===true) return void callback({id:doc.subject, username:credential.username, realm:this.realm});
				return void lengthyReturn(false);
			});
		});
	});
