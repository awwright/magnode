...
module.exports = function(db, transform, resources, render, callback){
	...
	*/
	function testReplace(err, _replaceDoc){
		...
		var revision = {};
		if(schema.put.projectionMapping){
		...
		if(replaceDoc){
			if(replaceDoc._id){
				where._id = replaceDoc._id;
			}
		}else{
			parsedData._id = revision._id = new ObjectId;
		}
		...
		function nextOperation(i){
			...
			if(op.insert){
				...
				db.remove(op.where, options, function(err, count){
					if(err) return void haveResponse(err);
					if(typeof op.updateCount=='number' && op.updateCount!==count){
						haveResponse(new Error('Mongo query should have removed '+op.updateCount+' documents, but only removed '+count));
						return;
					}
					nextOperation(i+1);
				});
	...
	function haveResponse(err){
		...
		if(resources.indexer){
			...
			if(request.method=='PUT'){
				resources.indexer['HTTPAuto_typeMongoDB_Put_Object'].emit(db, transform, resources, render, parsedData, schema, links);
				resources.indexer['HTTPAuto_typeMongoDB_Put_Operations'].emit(db, transform, resources, operations);
			}else if(request.method=='DELETE'){
				resources.indexer['HTTPAuto_typeMongoDB_Delete_Object'].emit(db, transform, resources, where);
			}
		}
