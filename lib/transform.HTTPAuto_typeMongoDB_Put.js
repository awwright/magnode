...
module.exports = function(db, transform, resources, render, callback){
	...
	function haveSchema(err, typeDoc){
		...
		if(collectionName){
			collection = resources['db-mongodb'].collection(collectionName);
		}else{
			collection = resources['db-mongodb-nodes'];
		...
		var selfTpl = linkTemplates.filter(function(v){ return v.rel==relns('self'); }).map(function(v){ return v.href; })[0];
		where = selfTpl && new Uritpl(selfTpl).fromUri(resources.variant.resource) || {};
		collection.find(where, testReplace);
	}
	/*
	Pre-update find():
	...
	*/
	function testReplace(err, replaceDoc){
		...
		var docEtag = replaceDoc[schema.etagField] && replaceDoc[schema.etagField].toString();
		...
		if(schema.put.insertRevision &&0){ //FIXME
