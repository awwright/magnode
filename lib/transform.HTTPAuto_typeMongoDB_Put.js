/*
e.g. Transform:HTMLBodyAuto_typeType_Put
	a view:ModuleTransform, view:Transform, view:PutTransform ;
	view:module "magnode/transform.HTMLBodyAuto_typeMongoDB_Put" ;
...
module.exports = function(db, transform, resources, render, callback){
	...
	var contentTypeProfile, operations=[];
	...
	authResources._auth = resources.access_token;
	console.log('authResources', authResources);
	console.log('http://magnode.org/UserSession', authResources['http://magnode.org/UserSession']);
	...
	function parseFormAuth(){
		authz.test(authResources, ['parse'], resources, function(authorized){if(authorized===true){
			...
			response.end("PUT THIS DATA:\n"+JSON.stringify(parsedData,null,"\t"));
			return void callback(null, {"http://magnode.org/HTTPResponse":200});

			haveRenderedForm(null, resources);
		}else{
			response.statusCode = 403;
			response.end("transform.HTMLBodyAuto_typeMongoDB_Put: `parse` <"+resources.resource+">: Denied\n");
	...
	function haveRenderedForm(err, rendered){
		if(err){
			...
			response.write("transform.HTMLBodyAuto_typeMongoDB_Put: Cannot parse JSON for field schema.\n");
	...
	function haveSchema(err, typeDoc){
		...
		function iterateTypeCheck(i){
			...
			authz.test(authTest, ['edit'], typeDoc, function(authorized){if(authorized===true){
				...
				response.statusCode = 403;
				response.end("transform.HTMLBodyAuto_typeMongoDB_Put: Edit <"+parsedData.subject+"> type <"+parsedData.type[i]+">: Denied\n");
				callback(null, {"http://magnode.org/HTTPResponse":403});
			}});
		}
	}
	...
	function haveResponse(err){
		...
		response.end("transform.HTMLBodyAuto_typeMongoDB_Put: Update <"+resources.resource+">\n");
		callback(null, {"http://magnode.org/HTTPResponse":303});
	}
}
