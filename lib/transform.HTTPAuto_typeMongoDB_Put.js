	...
	function haveSchema(err, typeDoc){
		...
		function writeInstance(instance, schema, options, ctx){
			...
			linkTemplates.forEach(function(v){
				...
				var statement = new rdfenv.createTriple(rdfenv.createNamedNode(resources.variant.resource), rdfenv.createNamedNode(v.rel), rdfenv.createNamedNode(tpl.fillFromObject(instance)));
			...
			if(schema.put.storeResource){
				...
				var selfTpl = schema.links.filter(function(v){ return v.rel=='self'; }).map(function(v){ return v.href; })[0];
