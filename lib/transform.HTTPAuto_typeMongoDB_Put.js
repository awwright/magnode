...
module.exports = function(db, transform, resources, render, callback){
	...
	*/
	function testReplace(err, _replaceDoc){
		...
		revision = {_id: undefined}; // this just puts _id at the top of the object when pretty-printed
				...
				if(dataum && dataum instanceof Array){
					dataum.forEach(function(v){
						...
						if(v.type=='CacheItem'){
							...
							var listcache = _cache.list = _cache.list || {};
							listcache[v.name] = v.value;
						}
					});
				}
				runOperations();
	...
	function haveResponse(err){
		...
		if(resources.indexer){
			...
			if(request.method=='PUT'){
				resources.indexer['MongoDB_Index_Put'].emit(db, transform, resources, render, revision, schema, links);
			}else if(request.method=='DELETE'){
				resources.indexer['MongoDB_Index_Delete'].emit(db, transform, resources, where);
			}
		}
