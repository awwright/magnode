	...
	function haveSchema(err, typeDoc){
		...
		function writeInstance(instance, schema, options, ctx){
			...
			if(schema.put.storeResource){
				...
				var state = {_id:null, _parent:instance.$id, _revision:revision._id};
				for(var k in instance) if(k[0]!='$' && revision[k]===undefined) revision[k] = instance[k];
				operations.push({collection:schema.put.storeResource, insert:revision, upsert:true, schema:schema, name:ctx.propertyPath});
			}
	...
	function runOperations(){
		...
		function nextOperation(i){
			...
			if(op.set){
				...
				var options = { upsert:op.update.upsert, multi:op.update.multi };
