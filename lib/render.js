...
Render.prototype.resolve = function resolve(target, input, useTransformTypes, depth){
	...
	for(var i=0; i<target.length; i++){
		...
		for(var j=0;j<transforms.length;j++){
			...
			var transformTypes = db.match(transform, rdf.rdfns("type")).map(function(v){return v.object.toString()});
		...
		for(var m=0; m<sList.length; m++){
			...
			for(var j=0;j<transforms.length;j++){
				...
				for(var a=0; a<matches.length; a++){
					...
					transformOptional.forEach(function(transformRequested){
						var solutionsOptional = [];
						var matches = render.search([transformRequested], sc.output, useTransformTypes, depth+1);
						// FIXME this logic won't work for >1 optional variants
						matches.forEach(function(solutioo){
							var scc = cloneSolution(sc); // Solution-Clone-Clone
							for(var k in solutioo.output) scc.output[k]=solutioo.output[k];
							scc.transforms = sc.transforms.concat(solutioo.transforms);
