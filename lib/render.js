...
function Render(db, renders){
	this.db = db || new rdf.TripletGraph;
	this.cache = {};
	...
	for(var i=0; i<target.length; i++){
		...
		for(var m=0; m<sList.length; m++){
			...
			for(var j=0;j<transforms.length;j++){
				...
				var transformDomain = db.getCollection(transformDomainFirst);
				var transformRange = db.match(transform, "http://magnode.org/view/range").map(function(v){return v.object});
				var transformNice = parseFloat(db.match(transform, "http://magnode.org/view/nice").map(function(v){return v.object})[0]) || 0;
				//console.log(pad(depth)+"* transform[test"+i+"]="+listURI([transform])+" domain:"+listURI(transformDomain)+"; range:"+listURI(transformRange)+" .");
				// Find the transforms for each of the required inputs
				...
				for(var a=0; a<matches.length; a++){
					...
					sc.transforms = s.transforms.concat(solution.transforms, transform);
					sc.nice = s.nice+solution.nice + transformNice;
					//console.log(pad(depth)+"Prepending: %j",transform);
					solutions.push(sc);
				}
				//break;
...
Render.prototype.applyTransform = function applyTransform(transform, input, callback){
	...
	function transformResult(err, v){
		...
		for(var m in v){
			if(input[m]===undefined) input[m]=v[m];
			else return void callback(new Error('Key '+m+' already exists'));
		}
		callback(null, input);
	}
}
...
Render.prototype.add = function add(fn, meta){
	...
	var db = this.db;
	var about = {
		// Maybe prepend http://magnode.org/view/
		a: ['http://magnode.org/view/Transform'].concat(m.type),
		'http://magnode.org/view/domain': {$list: m.domain},
		'http://magnode.org/view/range': m.range
	};
