...
function registerHandler(testResource, route, resources, view){
	...
	function routeResource(request, cb){
		...
		input.requestDataBuffer = {data:"", size:0};
		...
		// Now ask the database if it has the resource available
		testResource(input, function(cbOut){
			...
			function response(request, response){
				...
				// Go back to the database, ask it to process the resource
				cbOut(input, haveResources);
				function haveResources(resources){
					var req = Object.create(input);
					if(resources) for(n in resources) req[n]=resources[n];
					input.authz.test(null, 'get', req, function(authorized){
						if(authorized===true){
