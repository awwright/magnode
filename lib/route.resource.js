		...
		testResource(input, function(cbOut){
			...
			function response(request, response){
				...
				cbOut(input, function(a){
					...
					if(formatDocument){
						formatDocument = resources.rdf.resolve(formatDocument) || resources.rdf.resolve(':'+formatDocument);
						target = ['http://magnode.org/HTTPResponse'];
						if(formatDocument) target.unshift(formatDocument);
					}else{
						// TODO this should also generate a URL that uses the formatDocument block above instead
