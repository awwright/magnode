		...
		for(var i=0;i<fields.length;i++) (function(i){
			...
			render.render("http://magnode.org/DocumentHTML_Body", inputs, function(formatted){
				if(formatted['http://magnode.org/DocumentHTML_Body']){
					...
					result[i] = "<h3 title=\"a "+propertyTypes.join(",")+"\">"+title+"</h3><input type=\"text\" style=\"width:100%\" name=\"value.0.0."+i+"\" value=\""+htmlEscape(content)+"\">";
				}
				finished++;
				if(finished===fields.length){
					...
					result =
						'<form action="'+input.request.url+'" method="post">'
						+ result.join("")
