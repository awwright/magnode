...
module.exports.scanMongoCollection = function(db, dbSchema, render, cb){
	...
	function routeRequest(resource, found){
		...
		function foundRecord(schema, variants){
			//variants[rdf.environment.resolve(':Published')] = 1;
			// FIXME produce a custom menu here
			var menuItems = [];
			for(var n in requiredTypeMap){
				menuItems.push({label:n, requiredTypes:requiredTypeMap[n]});
			}
			variants['http://magnode.org/HTMLBodyBlock_ResourceMenu'] =
				'<ul>'
				+ menuItems.map(function(v){
					var menuRef = Object.create(variant);
					menuRef.requiredTypes = v.requiredTypes;
					return '<li><a href="'+escapeHTMLAttr(menuRef.toURI())+'">'+escapeHTML(v.label)+'</a></li>';
				}).join('')
				+ '</ul>';
