...
module.exports.scanMongoCollection = function(db, dbSchema, render, cb){
	...
	function routeRequest(resource, found){
		...
		for(var c in collections){
			collections[c].routes.forEach(function(route){
		...
		function findNext(i){
			...
			var route = v.route;
			if(!route.collection) return void found();
	...
	function updateSchema(collection, node){
		if(collections[collection]){
			...
			collections[collection].graph.forEach(function(t){
				render.db.remove(t);
			});
		}
		...
		if(options && options.page && options.page.type){
			var uri = node.id+'_Transform_Body'
		...
		addResource(node.id+'_Transform_PutForm', require('./transform.HTMLBodyAuto_typeMongoDB_Form'),
			...
			, view$range: ['http://magnode.org/HTMLBody_PutForm', 'http://magnode.org/HTMLBody', 'http://magnode.org/DocumentTitle']
			, view$nice: 1
			} );
		...
		addResource(node.id+'_Transform_Delete', require('./transform.HTTPAuto_typeMongoDB_Delete'),
			{ $context: context
	...
	function IndexMongoDBRoutes(fndb, transform, resources, render, document, schema, links){
		console.log('IndexMongoDBRoutes', links);
		function filter(u){
			// FIXME use the full URI
		...
		console.log('IndexMongoDBRoutes', 'REPROCESS THEM SCHEMA');
