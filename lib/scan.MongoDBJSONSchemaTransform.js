...
module.exports.scanMongoCollection = function(db, dbSchema, render, cb){
	...
	var routes = [];
	function route(resource, found){
		var nir = resource.split('?',1)[0];
		var m, r;
		routes.some(function(route){
			// Find the first match and stop
			m=route.template.fromUri(nir);
			if(m){
				r=route;
				return m;
			}
		});
		if(!m) return void found();
		// TODO Coerce m.filter into validating by m.route.schema here
		db.collection(r.collection).findOne(m, function(err, record){
			if(err) return void found(err);
			if(!record) return void found();
			var ret = {};
			ret[rdf.environment.resolve(':Published')] = 1;
			r.types.forEach(function(t){ ret[t] = record; });
			//ret['http://magnode.org/HTMLBody'] = '<h1>'+escapeHTML(record.label)+'</h1><pre>'+JSON.stringify(record,null,"\t")+'</pre>';
			return void found(null, ret);
		});
	}
	...
	function rsEach(node){
		...
		if(links instanceof Array){
			...
			if(node.describes) types.push(node.describes);
			links.forEach(function(v){
