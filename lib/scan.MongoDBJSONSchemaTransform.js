...
module.exports.scanMongoCollection = function(db, dbSchema, render, cb){
	function addTriple(s,p,o){
		// FIXME this is only a quick fix for convienence
	...
	function routeRequest(resource, found){
		// Ensure that every argument in the query string actually does something, otherwise MAYBE send a 404
		var variant = queryVariant(resource);
		var requiredTypes = variant.requiredTypes = variant.requiredTypes||{};
		if('edit' in variant.params){
			requiredTypes['http://magnode.org/HTMLBody'] = true;
			requiredTypes['http://magnode.org/HTMLBody_PutForm'] = true;
		}
		if('put.fn' in variant.params){
			requiredTypes['http://magnode.org/HTTPResponse_PutFn'] = true;
		}
		if('delete' in variant.params){
			requiredTypes['http://magnode.org/HTMLBody'] = true;
			requiredTypes['http://magnode.org/HTMLBody_DeleteForm'] = true;
		}
		if('delete.fn' in variant.params){
			requiredTypes['http://magnode.org/HTTPResponse_DeleteFn'] = true;
		}
		var schema = schemas[variant.resource];
		if(schema && schema.schema) schema = schema.schema;
		if(schema && 'new' in variant.params){
			// Search for a schema at variant.resource
			// Create an instance of it
			var instance = {};
			var types = [];
			if(schema.links instanceof Array){
				schema.links.forEach(function(v){
			...
			// Return it
			foundRecord(schema, types, instance);
		...
		function findNext(i){
			...
			db.collection(route.collection).findOne(v[0], function(err, record){
				...
				foundRecord(route.schema, route.types, record);
			});
		}
		function foundRecord(schema, types, record){
			var ret = {};
			ret.variant = variant;
			//ret[rdf.environment.resolve(':Published')] = 1;
			ret['http://magnode.org/HTMLBodyBlock_ResourceMenu'] = '<ul><li><a href="?">View</a></li><li><a href="?edit">Edit</a></li><li><a href="?edit">Edit Track Metadata</a></li><li><a href="?delete">Delete</a></li></ul>';
			types.forEach(function(t){ ret[t.fillFromObject(record)] = record; });
			// The schema may define URLs for which particular type will be generated out
			if(schema.views){
				var views = schema.views;
				for(var name in views){
					// If `name` exists in the `variant.params`, then append `views[name]` to `requiredTypes`
					if(typeof variant.params[name]==='string'){
						requiredTypes[views[name]] = true;
					}
				}
			}
			found(null, ret);
	...
	function rsEach(node){
		...
		if(options && options.page && options.page.type){
			...
			addResource(node.subject+'_Transform', require('./transform.HTMLBodyAuto_typeMongoDB'),
				...
				, a: ['view:Transform', 'view:GetTransform']
				, view$domain: {$list: [node.subject]}
				, view$range: ['http://magnode.org/HTMLBody', rdf.environment.createLiteral('view:info')]
				, view$nice: 1
				} );
		}
		...
		addResource(node.subject+'_Transform_Put_Stream', require('./transform.Document_typeStream'),
			...
			, view$domain: {$list: [rdf.environment.createLiteral('request:application/json;profile='+node.subject)]}
			, view$range: [rdf.environment.createLiteral('media:application/json;profile='+node.subject)]
			, view$nice: 0
			} );
		addResource(node.subject+'_Transform_Put_Parse', require('./transform.JSON_typeDocument'),
			{ $context: context
			, a: ['view:Transform', 'view:PutTransform']
			, view$domain: {$list: [rdf.environment.createLiteral('media:application/json;profile='+node.subject)]}
