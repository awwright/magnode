...
module.exports.scanMongoCollection = function(db, dbSchema, render, cb){
	...
	function addResource(s,f,o){
		...
		rdf.parse(o, s).graphify().forEach(function(t){ render.db.add(t); console.log(t.toString()); });
	}
	...
	function routeRequest(resource, found){
		...
		if('new' in variant.params){
			// FIXME do we really want to do this here?
			// If this resource is a schema, then generate a new instance of it
			var document = {};
			// TODO coerce `document` into validating against the schema
		}
		...
		if('delete.fn' in variant.params){
			requiredTypes['http://magnode.org/HTTPResponse_DelFn'] = true;
		}
		var matches = [];
		...
		function findNext(i){
			...
			db.collection(route.collection).findOne(v[0], function(err, record){
				...
				if(route.schema.views){
					var views = route.schema.views;
	...
	function rsEach(node){
		...
		if(links instanceof Array){
			...
			links.forEach(function(v){
				...
				routes.push({template:new Uritpl(v.href), pattern:v.href, types:types, collection:node.collection, definition:node, schema:schema});
			});
		}
