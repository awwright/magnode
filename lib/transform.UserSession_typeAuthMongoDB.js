...
module.exports = function(db, transform, input, render, callback){
	...
	if(!cred) return void callback(null, {"http://magnode.org/UserSession":null});

	cred.authenticateRequest(input['request'], function(err, auth){
		if(auth && auth.id && db){
			db.findOne({subject:auth.id, type:accountType}, userRecord);
		}else{
			return void callback(null, {"http://magnode.org/UserSession":null});
		}

		function userRecord(err, userDoc){
			if(err) return void callback(err);
			if(!userDoc) return void callback(null, {"http://magnode.org/UserSession":null});
