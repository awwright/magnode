...
module.exports = function(db, transform, input, render, callback){
	...
	if(correctToken!==fieldData._auth){
		...
		response.end("transform.HTMLBodyAuto_typeMongoDB_Delete: Update <"+input.resource+">: Denied (bad token)\n");
		return void callback(null, {"http://magnode.org/HTTPResponse":403});
	}
	var document, requestMethod, contentTypeProfile, operations=[];

	authz.test(null, "delete", input, function(authorized){if(authorized===true){
		...
		response.statusCode = 403;
		response.end("transform.HTMLBodyAuto_typeMongoDB_Delete: Update <"+input.resource+">: Denied\n");
