		...
		args.forEach(function(arg, i){
			if(arg.type=="literal"){
				...
				list.forEach(function(result, j){
					...
					var resourceTypes = db.match(result, rdf.rdfns('type')).map(function(v){return v.object});
					...
					render.render(arg.value.type, resources, arg.transformType||[], function(r){
						rendersRemaining--;
						if(!r) throw new Error('Could not create resource');
				...
				for(var j=0; j<arg.inputs.length; j++){
					...
					var resourceTypes = db.match(result, rdf$type).map(function(v){return v.object});
					for(var j=0;j<resourceTypes.length;j++) resources[resourceTypes[j]]=result;
				}
				render.render(arg.type, resources, arg.transformType||[], function(r){
					rendersRemaining--;
					if(!r) throw new Error('Could not create resource: '+require('util').inspect(resources));
