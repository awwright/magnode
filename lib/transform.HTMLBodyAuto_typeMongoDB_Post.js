		...
		function parseDocumentToSchema(schema, instance, operation, cb){
			...
			if(schema.type=='shadow'){
				...
				if(instance instanceof ObjectId){
					...
					//	if(err) throw err;
						console.log('INSERT SHADOW: ', instance);
						cb(id);
				...
				if(schema.updateDocument.select){
					...
					input['db-mongodb'].findOne({_id:instance._id}, function(err, doc){
						...
						console.log('READ {_id:'+doc._id+'} = ', doc);
						decendSchema(schema, instance, doc, function(value){
							console.log('UPDATE {_id:'+doc._id+'} = ', value);
							cb(id);
						});
					});
					...
					decendSchema(schema, instance, {}, function(value){
						...
						document = instance;
						if(id){
							console.log('UPDATE', {_id:id, _etag:etag}, {$set:instance});
							input['db-mongodb'].update({_id:id}, {$set:instance}, operationFinished);
						}else{
							console.log('INSERT', instance);
							input['db-mongodb'].insert(instance, true, operationFinished);
						}
