...
module.exports = function(db, transform, input, render, callback){
	...
	function haveSchema(err, typeDoc){
		...
		console.log('If-match ETag: ', etag);
		...
		function writeInstance(instance, schema, options, ctx){
			...
			if(schema.put.storeRevision){
				var _id = instance.$etag;
				var revision = {_id: null, _parent: instance._id};
				for(var k in instance) revision[k] = instance[k];
				delete revision.$etag;
				revision._id = new ObjectId;
				var where = {subject:revision.subject};
				...
				operations.push({collection:'nodes', where:where, update:{$rename:{subject:'_subject'}} });
	...
	function runOperations(){
		function nextOperation(i){
			...
			if(op.set){
				...
				db.update(op.where, op.update, function(err){
					if(err) throw err;
					nextOperation(i+1);
				});
			}else{
