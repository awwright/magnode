...
module.exports = function(db, transform, input, render, callback){
	...
	function renderMenu(){
		...
		var menuItems = [];
		// FIXME query a "menu items" index calculated from "menulink" link relations gleaned from all the respective collections
		var srcdb = input['db-mongodb-nodes'];
		srcdb.find(query, {_id:1,subject:1,label:1,menu:1}).each(function(err, doc){
			if(err) rsEnd(err);
