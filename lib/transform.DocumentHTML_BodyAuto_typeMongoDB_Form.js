...
function renderField(name, type, value){
	...
	switch(type.type){
		case 'ObjectId':
			return '<input name="value.'+htmlEscape(name)+'" value="'+htmlEscape(value)+'"/><input type="hidden" name="type.'+htmlEscape(name)+'" value="ObjectId"/>';
		case 'string':
			switch(type.format){
				case 'uri':
				case 'email':
				...
				default:
					return '<textarea name="value.'+htmlEscape(name)+'">'+htmlEscape(value)+'</textarea><input type="hidden" name="type.'+htmlEscape(name)+'" value="string"/>';
			}
		case 'array':
		case 'json':
			return '<textarea name="value.'+htmlEscape(name)+'">'+htmlEscape(JSON.stringify(value,null,"\t"))+'</textarea><input type="hidden" name="type.'+htmlEscape(name)+'" value="json"/>';
		case 'shadow':
			return '<div><input type="password" name="value.'+htmlEscape(name)+'" value=""/></div><div><input type="password" name="confirm.'+htmlEscape(name)+'" value=""/></div><input type="hidden" name="type.'+htmlEscape(name)+'" value="shadow"/>';
		default:
			return '<pre>'+util.inspect(value)+'</pre>';
	}
...
module.exports = function(db, transform, input, render, callback){
	...
	input['db-mongodb-schema'].findOne({subject:templateInputType[0]}, function(err, schema){
		...
		for(var n in properties){
			...
			fields.push("<dt>"+htmlEscape(fieldTitle)+"</dt><dd>"+renderField(n, properties[n], node[n]||"")+"</dd>");
			fieldNames.push(n);
		}
