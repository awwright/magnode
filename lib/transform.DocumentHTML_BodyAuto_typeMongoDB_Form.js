...
module.exports = function(db, transform, input, render, callback){
	...
	function haveRenderedFields(err, renderedFields, properties){
		...
		for(var n in properties){
			...
			fields.push("<dt>"+htmlEscape(fieldTitle)+"</dt><dd>"+renderedFields[n]+"</dd>");
			fieldNames.push(n);
		}

		for(var n in node){
			if(properties[n]) continue;
			fields.push('<dt>'+htmlEscape(n)+'</dt><dd><textarea name="value.'+htmlEscape(name)+'">'+htmlEscape(JSON.stringify(value,null,"\t"))+'</textarea><input type="hidden" name="format.'+htmlEscape(name)+'" value="json"/></dd>');
			fieldNames.push(n);
		}
