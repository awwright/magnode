	...
	function renderFields(properties, node, fieldList, renderedFields, cb){
		...
		render.render(targetType, input, transformTypes, function(err, res){
			...
			if(res && res[targetType]){
				...
				renderedFields[fieldName] = '<pre class="field-default">'+escapeHTML(util.inspect(node[fieldName]))+'</pre><input type="hidden" name="format.'+escapeHTML(fieldName)+'" value="noop"/>';
			}
