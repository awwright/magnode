...
module.exports = function(db, transform, resources, render, callback){
	...
	function haveRenderedForm(err, res){
		...
		if(title){
			...
			if(href) title = '<a href="'+escapeHTMLAttr(relativeuri(resources.rdf, resources.request.uri, href))+'">'+title+'</a>';
			title = '<h2>'+title+'</h2>';
		}
		...
		var output = {};
		outputTypes.forEach(function(v){output[v]=result;});
		output['http://magnode.org/DocumentTitle'] = node.label || node.title;
		callback(null, output);
	}
}
