...
return function HTMLBodyField_typeNode_FieldObject(db, transform, resources, render, callback){
	...
	function haveRenderedFields(err){
		...
		if(dlItems.length){
			...
			if(outputType=='FormTransform') result += '<dt>'+renderedFields[baseName+'new.name']+'<input type="hidden" name="'+escapeHTMLAttr(baseName+'new.format')+'" value="ObjectProperty"/></dt><dd>'+renderedFields[baseName+'new.value']+'</dd>';
			result += '</dl>';
		}
		if(!result){
			result += '<i>empty</i>';
