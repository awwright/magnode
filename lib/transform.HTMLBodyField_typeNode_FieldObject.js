...
return function HTMLBodyField_typeNode_FieldObject(db, transform, resources, render, callback){
	...
	var fieldNames = [];
	var renderedFields = {};
	// FIXME maybe we should figure out how to preserve the original value, even if it won't validate against the schema later
	if(inputResource.type==='array' && !(value instanceof Array)) value = [value];
	if(value instanceof Array){
		...
		if(outputType=='PutFormTransform' && additionalItems){
		...
		if(outputType=='PutFormTransform' && inputResource.additionalProperties!==false){
	...
	function renderFields(fieldList){
		...
		render.render(targetType, input, transformTypes, function(err, res){
			...
			if(res && res[targetType]){
				...
				if(outputType=='PutFormTransform') renderedFields[field.name] += '<input type="hidden" name="'+escapeHTMLAttr(field.name+'.format')+'" value="noop"/>';
			}
	...
	function haveRenderedFields(err){
		...
		if(olItems){
			...
			if(outputType=='PutFormTransform') classNames.push('field-array-new');
			result += '<ol class="'+classNames.join(' ')+'">';
			result += olItems.map(function(n){ return '<li>'+renderedFields[n]+'</li>'; }).join('');
			if(outputType=='PutFormTransform') result += '<li>'+renderedFields[baseName+'new']+'</li>';
			result += '</ol>';
		}
		if(dlItems){
			var classNames = ['field-object'];
			if(renderedFields[baseName+'new.name']) classNames.push('field-object-new');
			result += '<dl class="'+classNames.join(' ')+'">';
			result += dlItems.map(function(n){
				var f = (outputType=='PutFormTransform'&&n.p)?('<input type="hidden" name="'+escapeHTMLAttr(n.p+'.format')+'" value="ObjectProperty"/>'):'';
		...
		if(outputType=='PutFormTransform'){
