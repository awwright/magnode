...
module.exports = function(db, transform, input, render, callback){
	...
	function haveRenderedFields(err, renderedFields, properties){
		...
		for(var n in node){
			...
			fields.push('<dt>'+escapeHTML(n)+'</dt><dd><textarea name="'+escapeHTMLAttr(baseName+name+'.value')+'">'+escapeHTML(JSON.stringify(value,null,"\t"))+'</textarea><input type="hidden" name="'+escapeHTMLAttr(baseName+name+'.format')+'" value="json"/></dd>');
			fieldNames.push(n);
		}
