...
module.exports = function(db, transform, input, render, callback){
	...
	function haveRenderedFields(err, renderedFields, properties){
		...
		for(var n in node){
			...
			fields.push('<dt>'+escapeHTML(n)+'</dt><dd><textarea name="'+escapeHTMLAttr(baseName+n+'.value')+'" class="field-json">'+escapeHTML(JSON.stringify(node[n],JSONReplacer,"\t"))+'</textarea><input type="hidden" name="'+escapeHTMLAttr(baseName+n+'.format')+'" value="JSON"/></dd>');
			fieldNames.push(n);
		}
