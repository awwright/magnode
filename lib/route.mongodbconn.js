		...
		if(hier[0]===''){
			...
			if(keyType===''){
				...
				db.collection(collectionName).indexes(function(err, indexes){
					...
					var result = {'http://magnode.org/MongoDBCollection': {db:db, collection:collectionName, collectionName:collectionName}};
					result['http://magnode.org/HTMLBodyBlock_ResourceMenu'] = [];
					callback(null, result);
				});
				return;
			}else if(keyType==='schema'){
				// Return the collection
				db.collection(collectionName).indexes(function(err, indexes){
					if(!indexes || !indexes.length) return void callback();
					var result = {'http://magnode.org/MongoDBCollection_Schema': {db:db, collection:collectionName, collectionName:collectionName}};
					result['http://magnode.org/HTMLBodyBlock_ResourceMenu'] = [];
					callback(null, result);
				});
				return;
			...
			db.collection(collectionName).findOne(query, function(err, document){
				...
				var result = {'http://magnode.org/MongoDBDocument': resource};
				callback(null, result);
			});
		}
