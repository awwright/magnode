...
module.exports = function(db, transform, resources, render, callback){
	...
	resources['db-mongodb-schema'].findOne({subject:resourcesTypes[0].toString()}, function(err, typeDoc){
		if(!node.subject) node.subject=node.resource;
		var schema = typeDoc&&typeDoc.schema || typeDoc;
	...
	function haveRenderedForm(err, res){
		...
		var subject = resources.resource.split('?',1)[0];
		var result =
			'<form action="'+escapeHTMLAttr(resources.request.url)+'" method="post">'
			+ form
			+ '<input type="hidden" name="_id" value="'+escapeHTMLAttr(node._id||"")+'"/>'
			+ '<input type="hidden" name="_subject" value="'+escapeHTMLAttr(subject)+'"/>'
