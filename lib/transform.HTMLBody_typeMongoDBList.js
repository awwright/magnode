...
function transform(db, transform, resources, render, callback){
	...
	function renderField(field, record){
		if(field.text_content_field){
			return escapeHTML(record[field.text_content_field]);
		}else if(field.html_content_field){
			return record[field.html_content_field];
		}else{
	...
	function nextRecord(err, result){
		...
		if(recordType==='li'){
			html += '<li>';
			html += renderField(query.field || query.fields[0], result);
			html += '</li>';
		}else if(recordType==='tr'){
			html += '<tr>';
			query.fields.forEach(function(field){
				html += '<td>'+renderField(field, result)+'</td>';
			});
			html += '</tr>';
			...
			query.fields.forEach(function(field){
				...
				html += '<dd>'+renderField(field, result)+'</dd>\n';
			});
			html += '</dl><hr/>';
