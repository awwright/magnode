...
function transform(db, transform, resources, render, callback){
	...
	function renderField(field, record, linkRels){
		...
		if(field.link_href_field && record[field.link_href_field]){
			...
			html = '<a href="'+escapeHTML(relativeURI(rdfenv, record[field.link_href_field]))+'">'+html+'</a>';
			...
			html = '<a href="'+escapeHTML(relativeURI(rdfenv, linkRels[relUri]))+'">'+html+'</a>';
		}
		return html;
	}
	function nextRecord(err, result){
		...
		if(recordType==='li'){
			...
			var self = relativeURI(rdfenv, linkRels[relns('self')]);
