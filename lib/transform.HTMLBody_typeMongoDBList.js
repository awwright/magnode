...
var relns = require('rdf').ns('http://www.iana.org/assignments/relation/');
...
function transform(db, transform, resources, render, callback){
	...
	var pager = resources.variant.pager = resources.variant && resources.variant.pager || {};
	pager.limit = options.limit = pager.limit || query.pager.limit;
	if(pager.offset){
	...
	function renderField(field, record, linkRels){
		if(field.dereferenceField){
			var cache = record && record._cache && record._cache.list;
			var d = field.dereferenceField;
			var key = d.localProperty+' '+d.foreignCollection+' '+d.remoteProperty;
			record = cache && cache[key] || {};
		}
		var html;
		...
		if(html=='' && field.text_content_ifempty){
			html = field.text_content_ifempty;
		}
		...
		if(field.link_href_field && record[field.link_href_field]){
			...
			html = '<a href="'+escapeHTML(relativeURI(rdfenv, resources.request.uri, record[field.link_href_field]))+'">'+html+'</a>';
			...
			html = '<a href="'+escapeHTML(relativeURI(rdfenv, resources.request.uri, linkRels[relUri]))+'">'+html+'</a>';
		}
		return html;
	}
	function nextRecord(err, result){
		...
		linkTemplates.forEach(function(v){
			function filter(w){ return (w in result); }
			if(!v.tpl.varNames.every(filter)) return;
		...
		if(recordType==='li'){
			...
			var self = relativeURI(rdfenv, resources.request.uri, linkRels[relns('self')]);
			html += '<h2><a href="'+escapeHTML(self)+'">'+escapeHTML(result.label)+'</a></h2>\n';
			...
			html += '<div class="link-wrapper"><ul class="links inline"><li class="node-readmore first"><a href="'+escapeHTML(self)+'" rel="tag" title="$title">Read more: $title</a></li></ul></div>';
			html += '</div>';
			...
			render.render(targetType, input, transformTypes, function(err, res){
				...
				if(res && res[targetType]){
					...
					html = '<pre class="field-default">No result for '+escapeHTML(targetType)+'</pre>';
				}
...
transform.about = {
	...
	type: ['http://magnode.org/view/Transform', 'http://magnode.org/view/GetTransform', 'http://magnode.org/view/Core'],
