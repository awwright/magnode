...
module.exports.prototype.applyTransforms = function(applyTransforms, input, cb){
	...
	if(transform){
		this.applyTransform(transform, input, function(formatted){
...
module.exports.prototype.render = function(target, input, useTransformTypes, callback){
	...
	if(this.cache){
		...
		if(self.cache[key]) callback(self.cache[key]);
		else this.applyTransforms(transformSeries.transforms, input, function(out){self.cache[key]=out; callback(out);});
	}else{
