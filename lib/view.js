...
module.exports.render = function(db, renders, target, input, callback){
	...
	input.log("input.resource="+listURI([input.resource])+" a:"+listURI(resourceTypes)+" .");
				...
				if(renders[transformTypes[j]]) (function(j){
					...
					for(var k=0;k<requiredInputs.length;k++){
						...
						transform(requiredInputs[k], input, depth+1, parents.concat([transformTypes[j]]), function(v){ // stepCallback
							...
							//sinput.log(pad(depth)+requiredInputs[k]+" output:"+util.inspect(v));
					...
					endCallback = function(){ // endCallback
						...
						renderFinished = true;
						transformCall(db, transforms[i], childInput, callback);
					};
					if(pending===0 && endCallback) endCallback();
				})(j);
			}
...
module.exports.prototype.render = function(target, input, callback){
	module.exports.render(this.db, this.renders, target, input, callback);
}
