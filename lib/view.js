	...
	function transform(target, input, depth, parents, callback){
		...
		for(var i=0;i<transforms.length;i++){
			...
			if(!useTransformTypes.every(function(v){return transformTypes.indexOf(v)!==-1})){
				input.log(pad(depth)+" --- Not member of complete set "+listURI(useTransformTypes));
				continue;
			}
			// The transform we're trying has types that tell us how to process the transform.
			for(var j=0;j<transformTypes.length;j++){console.log(pad(depth)+" ---  transformTypes[t"+j+"]=%s renders[transformTypes[t"+j+"]]=%s", transformTypes[j], renders[transformTypes[j]]&&renders[transformTypes[j]].name||typeof(renders[transformTypes[j]]));
