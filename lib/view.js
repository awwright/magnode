...
module.exports.prototype.applyTransform = function(transform, input, callback){
	...
	var transformCacheable = db.filter({subject:transform, predicate:"http://magnode.org/view/cache"}).map(function(v){return v.object})[0];
	...
	cache.get(hashhex||null, function(result){
		if(result){
			...
			callback(new Error('No function to process transform <'+transform+'> found'));
		}
	});
