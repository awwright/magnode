...
module.exports.prototype.applyTransform = function(transform, input, useTransformTypes, callback){
	var self = this;
	var db = this.db;
	...
	console.log(" - transform[t"+i+"]="+listURI([transform])+" a:"+listURI(transformTypes)+"; domain:"+listURI(transformDomain)+"; range:"+listURI(transformRange)+" .");
	// The transform must be an instance of all the types listed in useTransformTypes
	if(!useTransformTypes.every(function(v){return transformTypes.indexOf(v)!==-1})){
		console.log(" --- Not member of complete set "+listURI(useTransformTypes));
		return;
	}
	...
	for(var j=0;j<transformTypes.length;j++){
		...
		console.log((transformCall.name||"transformCall")+"(db, '"+transform+"', input, self, callback)");
		transformCall(db, transform, input, self, function(v){
			var childInput = {};
...
module.exports.prototype.render = function(target, input, useTransformTypes, callback){
	...
	function transform(target, input, depth, parents, callback){
		if(depth>10){callback({overflow:depth});return;}
		...
		for(var i=0;i<transforms.length;i++){
			...
			// The transform must be an instance of all the types listed in useTransformTypes
			if(!useTransformTypes.every(function(v){return transformTypes.indexOf(v)!==-1})){
				input.log(pad(depth)+" - transform[t"+i+"]="+listURI([transforms[i]])+" SKIPPED a:"+listURI(transformTypes)+" not a "+listURI(useTransformTypes));
				continue;
			}
			input.log(pad(depth)+" - transform[t"+i+"]="+listURI([transforms[i]])+" a:"+listURI(transformTypes)+"; domain:"+listURI(transformDomain)+"; range:"+listURI(transformRange)+" .");
			// The transform we're trying has types that tell us how to process the transform.
			for(var j=0;j<transformTypes.length;j++){
				console.log(pad(depth)+" ---  transformTypes[t"+j+"]=%s renders[transformTypes[t"+j+"]]=%s", transformTypes[j], renders[transformTypes[j]]&&renders[transformTypes[j]].name||typeof(renders[transformTypes[j]]));
				// This is a way of processing the transform.
				if(renders[transformTypes[j]]){ (function(transforms, i, renders, transformTypes, j){
					renderPending++;
					// Copy the inputs for sub-renders
					var childInput = {};
					for(k in input) childInput[k]=input[k];
					...
					for(var k=0;k<requiredInputs.length;k++){
						input.log(pad(depth)+" - transform[t"+i+"]input["+k+"]="+listURI([requiredInputs[k]])+" typeof:"+typeof(input[requiredInputs[k]])+" .");
					...
					endCallback = function(){ // endCallback
						...
						renderFinished = true;
						console.log(pad(depth)+(transformCall.name||"transformCall")+"(db, '"+transforms[i]+"', childInput, self, callback)");
						transformCall(db, transforms[i], childInput, self, callback);
					};
					if(pending===0 && endCallback) endCallback();
				})(transforms, i, renders, transformTypes, j); break; }
			}
		}
