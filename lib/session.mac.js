...
function base64url(base64str){
	return base64str.replace(/=/g,'').replace(/\+/g,'-').replace(/\//g,'_');
}

module.exports = function MACSession(config){
...
module.exports.prototype.authenticateToken = function authenticateToken(token, callback){
	...
	hash.update(body);
	// Compare the calculated string against the token as provided -- it's opaque, remember?
	if(base64url(hash.digest("base64"))!==tokenParts[0]) return void callback(null);
...
module.exports.prototype.createSession = function(user){
	...
	var hash = crypto.createHmac(this.hmacHash, this.secret());
	hash.update(body);
	var token = base64url(hash.digest("base64")+"."+body.toString('base64'));
