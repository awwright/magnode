...
module.exports.scanMongoCollection = function(db, transformDb, cb){
	...
	db.find({type:"http://magnode.org/MongoDBJSONSchema"}).forEach(function(node){
		...
		if(render && render.page && render.page.type=='jade' && render.page.file){
			...
			addTriple(uri, 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type', 'http://magnode.org/view/Jade');
			addTriple(uri, 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type', 'http://magnode.org/view/ViewTransform');
			addTriple(uri, 'http://magnode.org/view/file', render.page.file);
			addTriple(uri, 'http://magnode.org/view/domain', node.subject);
			addTriple(uri, 'http://magnode.org/view/range', 'http://magnode.org/DocumentHTML_BodyRaw');
		...
		addTriple(uri, 'http://magnode.org/view/domain', 'http://magnode.org/UserMenu');
