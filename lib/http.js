...
function handleRequest(request, response, route, resources, view){
	...
	var resource = resources.rdf.resolve(path.substring(1)) || url.resolve(prefix, path);
	...
	function haveResource(result){
		var requestenv = Object.create(resources);
		...
		var input = Object.create(requestenv);
		...
		if(input.createNew){
			...
			input.node = {type:[resource], subject:subject};
			// Set pre-defined values...
			// TODO use a transform for this, maybe (pending security issues)
			var urlData = url.parse(input.request.url, true);
		...
		request.on('data', function(cz){
			input.requestDataBuffer.size += cz.length;
			if(input.requestDataBuffer.expected>=0 && input.requestDataBuffer.size>input.requestDataBuffer.expected){
		...
		if(request.headers['content-type']){
			input.requestDataBuffer.expected = parseInt(request.headers['content-length']);
			if(!(input.requestDataBuffer.expected >= 0)){
				...
				return false;
			}else if(input.requestDataBuffer.expected > 32*1024*1024){
			...
			input.requestDataBuffer.expected = 0;
		}
		...
		input.authz.test(null, 'get', input, function(authorized){
			if(authorized===true){
				if(result) for(n in result) input[n]=result[n];
				renderResources();
			}else{
				response.statusCode = 401;
				input['http://magnode.org/Unauthorized'] = {subject: resource};
				renderResources();
			}
		});
