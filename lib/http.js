...
function handleRequest(request, response, route, staticresources, view){
	...
	function haveResource(dbresult){
		var variant = dbresult.variant || queryVariant(request.url);
		...
		requestenv.createNew = variant.createNew;
		...
		switch(request.method){
			case "GET":
			case "HEAD":
				if(variant.editable||variant.createNew) useTransformTypes.push("http://magnode.org/view/PutFormTransform");
				else if(variant.showDelete) useTransformTypes.push("http://magnode.org/view/DeleteFormTransform");
		...
		function renderResources(){
			...
			var formatDocument = variant.type;
			...
			var outputOptions = outputFound.filter(function(res){
				...
				function filterType(v){
					if(v===variant.type) return true;
				}
				if(variant.type && !Object.keys(res.output).some(filterType)){
					return false;
				}
				// Enforce match of media types, if asked for
				function filterMedia(v){
					if(v.substr(0,6)!=='media:') return;
					var cmp = contenttype.mediaCmp(contenttype.parseMedia(variant.media), contenttype.parseMedia(v.substring(6)));
					if(typeof cmp!='number') return;
					if(cmp >= 0) return true;
				}
				if(variant.media && !Object.keys(res.output).some(filterMedia)){
					return false;
				}
				return true;
			});
			var filtered = outputOptions.filter(function(res){
				return Object.keys(res.output).some(function(v){
					// I forget what this does exactly, it appears to be allowing you to
					// use <?media:text/json> to require a particular media type
