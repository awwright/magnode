...
function handleRequest(request, response, route, staticresources, view){
	...
	function lookupResource(err, answers){
		...
		if(err){
			...
			response.end("404 Not Found:\nRoute error: No matching route for "+resource+"\n\nRoutes:\n"+util.inspect(route.routes));
		}else if(!answers){
			response.writeHead(500, {'Content-Type': 'text/plain', 'Server':Server});
	...
	switch(request.method){
		...
		case "OPTIONS":
			...
			route.route(request, lookupResource);
			break;
		case "PUT":
			// PUT operates on the resource provided in the entity-body. It will be
			// up to the called transforms to save that entity to the database.
			response.statusCode = 501; // or 501 if a server error
			response.setHeader("Content-Type", "text/plain");
			response.end("Cannot handle PUT yet...\n");
			break;
		case "TRACE":
