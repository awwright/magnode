...
function handleRequest(request, response, route, staticresources, render){
	...
	var requestLine = request.uri || request.url;
	var resource;
	if(requestLine[0]=='/'){ // origin-form
		resource = 'http://' + req.headers['host'] + requestLine;
	...
	resource = (resourceAbs&&resourceAbs[1]) || resourceLong || resource;
			...
			if(request.headers['content-type']){
				...
				if(media.type=='application/json' && media.params.profile){
					...
					require('./requestbody').readRequestBody(request, 30000, function(err, body){
						try {
							resources[media.params.profile] = JSON.parse(body);
						}catch(e){
	...
	function haveResource(dbresult){
		...
		applyTransforms = applyTransforms.map(function(v){return staticresources.rdf.resolve(v);});
