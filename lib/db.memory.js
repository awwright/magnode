...
module.exports.prototype = {
	...
	getCollection: function(subject, callback){
		...
		while(rest && rest!=nil){console.log("collectionFirst["+rest+"]="+require('util').inspect(collectionFirst[rest]));
	...
	evaluateQuery: function(queryGraph, querySubject, objectBindings){
		...
		function isVariable(v){return v.resolve().value.substr(0,sp_.length)==sp_};
		function varName(v){return v.resolve().substr(0,sp_.length)==sp_ && v.resolve().replace(sp_, "")};
		...
		var resultVars = queryGraph.filter({subject:querySubject, predicate:"sp:resultVariables"})[0].object;
