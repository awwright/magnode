...
module.exports = function(db, transform, input, render, callback){
	...
	var inputElement = input['http://magnode.org/FormFieldElementArray'];

	var arrLength = parseInt(inputData[inputElement.name+'.length']);
	if(!(arrLength>=0)) return callback(new Error('Invalid Array length'));
	var itemFormat = inputData[inputElement.name+'.itemformat'];
	if(!itemFormat) return callback(new Error('Invalid Array itemformat: '+itemFormat));
	var itemType = url.resolve('http://magnode.org/',itemFormat);
	var value = [];
	...
	function parseFields(fieldItem){
		...
		function haveParsedField(err, resources){
			...
			if(fieldData){
				value[fieldItem] = fieldData;
			}else{
				return callback(new Error('No parsed field returned for '+inputElement.name+'.'+fieldItem));
			}
...
module.exports.about =
	...
	, 'view:domain': {$list:['type:FormFieldData','type:FormFieldElementArray']}
